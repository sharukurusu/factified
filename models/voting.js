/**
 * From mongoose-voting npm, added additional functions
 * Module dependencies.
 */
...
function voting (schema, options) {
  ...
  schema.methods.upvoteRel = function upvote(user, fn) {
    // Reset vote if existed
    this.relevance.negative.pull(user);

    // Upvote
    this.relevance.positive.addToSet(user);
  ...
  schema.methods.downvoteRel = function downvote(user, fn) {
    // Reset vote if existed
    this.relevance.positive.pull(user);

    // Downvote
    this.relevance.negative.addToSet(user);
